...
A protocol change reverses which of two abstract design methods performs best.
The planner must decide how much old experimental history should influence the
new regime. We compare:

1. context-free: all biological history counts equally;
2. recency-only: old observations decay exponentially;
3. context+recency: observations also lose weight when protocol context differs.

Run:
    python backtest/run_context_benchmark.py --trials 400
"""
...
def _posteriors(
    ...
    if planner == "recency-only":
        # Preserve timestamps but deliberately normalize context so this arm
        # isolates temporal decay from context mismatch.
        normalized = [
            ExperimentObservation(
                method=o.method,
                hit=o.hit,
...
def run_trial(seed: int, planner: str) -> TrialResult:
    ...
    for round_index in range(1, N_ROUNDS + 1):
        ...
        posts = _posteriors(planner, observations, now)
...
def run_benchmark(trials: int) -> dict[str, dict[str, float]]:
